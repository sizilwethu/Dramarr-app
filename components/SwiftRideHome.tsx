...
import { MapPin, Star, Car, ShieldCheck, Navigation, Phone, CheckCircle2, Users, MoreHorizontal, ChevronLeft } from 'lucide-react';
          ...
          <>
            <div className="bg-slate-900/50 rounded-3xl p-5 mb-8 border border-white/5">
              <div className="flex items-center gap-4 mb-4">
                ...
                <input value={pickup} onChange={e => setPickup(e.target.value)} className="bg-transparent border-none text-sm font-bold text-white outline-none w-full" placeholder="Where from?" />
              </div>
              <div className="h-px bg-slate-800 w-full my-3 ml-7" />
              <div className="flex items-center gap-4">
                <MapPin className="text-red-500" size={18} />
                <input value={destination} onChange={e => setDestination(e.target.value)} className="bg-transparent border-none text-sm font-bold text-white outline-none w-full" placeholder="Where to?" />
              </div>
            </div>
            ...
            <button onClick={handleRequest} disabled={!destination} className="w-full bg-blue-600 text-white py-5 rounded-3xl font-black uppercase text-sm tracking-widest shadow-2xl shadow-blue-500/20 active:scale-95 transition-all disabled:opacity-20">
